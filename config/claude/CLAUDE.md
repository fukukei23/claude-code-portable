...
## 環境
- WSL2 Ubuntu / ホーム: //wsl.localhost/Ubuntu/home/__USERNAME__/
- タイムアウト: 10分。長時間タスクは分割
- シェルの $HOME は /c/Users/yn441 に解決されるため、WSLファイルは `//wsl.localhost/Ubuntu/home/__USERNAME__/` で参照
- シークレット管理: `~/.secrets.env` に一元化。`.env` は作成しない（`.env.example` のみ残す）

## 実行環境の注意（Windows + WSL2の2層構造）
スクリプト・ファイルはWSL側（`/home/__USERNAME__/`）にあるが、Cronや自動実行はWindowsのbashで動く。
...
## セッション開始バナー（必須）
最初の返答冒頭で必ず `wsl bash -c "bash /home/__USERNAME__/.claude/scripts/session/startup-banner.sh"` を実行し、その出力をそのまま貼ること。
...
## SSOT（共通知識ベース）
- 場所: `//wsl.localhost/Ubuntu/home/__USERNAME__/projects/obsidian-ssot/`
