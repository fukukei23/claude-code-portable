...
CLAUDE="/home/__USERNAME__/.local/share/fnm/node-versions/v22.22.2/installation/bin/claude"
...
if [[ "${1:-}" == "--auto" ]]; then
    ...
    echo "GitHub Issues を取得中: __USERNAME__23/$REPO ..."

    ISSUES=$(python3 - <<EOF
import yaml, json, urllib.request

hosts = yaml.safe_load(open('/home/__USERNAME__/.config/gh/hosts.yml'))
token = hosts['github.com']['oauth_token']

url = f'https://api.github.com/repos/__USERNAME__23/$REPO/issues?state=open&per_page=50'
...
import os
candidates = [
    f'/home/__USERNAME__/projects/$REPO',
    f'/home/__USERNAME__/projects/{\"$REPO\".replace(\"-\",\"_\")}',
]
for c in candidates:
    if os.path.isdir(c):
        print(c)
    ...
    print(f'/home/__USERNAME__/projects/$REPO')
")
...
setsid bash /home/__USERNAME__/.claude/scripts/auto-dev/run-issue.sh "$FIRST_ISSUE" &
echo "起動完了 (PID: $!)"
